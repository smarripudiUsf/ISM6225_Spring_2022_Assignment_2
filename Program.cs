        ...
        */
        ...
        public static int FindLucky(int[] arr)
        {
            try
            {
                ...
                {   // for each val in the dict
                    if (kp.Value == kp.Key)
                    {         // if it's a lucky number, and greater one than prev llucky no, store it
                        if (k < kp.Key)
                            k = kp.Key;
                    }
                }
        ...
        */
        ...
        public static string GetHint(string secret, string guess)
        {
            try
            {
                ...
                int cows = 0;
                for (int i = 0; i < len; i++)
                {            // calculate the number of bulls: if same char in both string( check in dict), then increment bulls
                    if (secret[i] == guess[i])
                    {
                        bulls++;
